...
export async function syncAutostartDesktop(appId, appName, days, startTime, endTime, execCommand) {
    ...
    if (desktopFile.query_exists(null)) {
        ...
        const origMatch = content.match(/^X-(?:GNOME-Scheduler|AppStartTime)-OriginalExec=(.*)$/m);
        if (origMatch) {
...
X-GNOME-Scheduler-OriginalExec=${cleanExec}
X-GNOME-Scheduler-Managed=true
Type=Application
Name=${appName}
Exec=${execLine}
Terminal=false
...
export async function removeAutostartDesktop(appId) {
    ...
    if (content.includes('X-GNOME-Scheduler-Managed=true') || content.includes('X-AppStartTime-Managed=true')) {
        await deleteFileAsync(desktopFile);
        return;
    }

    const matchOrig = content.match(/^X-(?:GNOME-Scheduler|AppStartTime)-OriginalExec=(.*)/m);
    if (matchOrig) {
...
Terminal=false
NoDisplay=true
X-GNOME-Scheduler-Extension=true
X-GNOME-Autostart-enabled=true
`;
